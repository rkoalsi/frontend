...
const CustomerLoginsPage: React.FC = () => {
    ...
    return (
        <Box sx={{ p: { xs: 2, md: 4 }, maxWidth: 1200, mx: 'auto' }}>
            {/* Header */}
            <Box display="flex" justifyContent="space-between" alignItems="center" mb={4} sx={{ pb: 3, borderBottom: `1px solid ${theme.palette.divider}` }}>
                <Box>
            ...
            <Paper elevation={0} sx={{ p: 3, mb: 3, borderRadius: 3, border: `1px solid ${theme.palette.divider}` }}>
                <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr auto auto' }, gap: 2, alignItems: 'center' }}>
                    ...
                    <Button variant="outlined" startIcon={<Refresh />} onClick={() => { setSearchTerm(''); setStatusFilter(''); }} sx={{ borderRadius: 2, height: 56, textTransform: 'none', fontWeight: 600, whiteSpace: 'nowrap' }}>
                        Clear Filters
                    </Button>
                </Box>
            </Paper>
            ...
            <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="md" fullWidth PaperProps={{ sx: { borderRadius: 3 } }}>
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                            ...
                            <Paper elevation={0} sx={{ p: 3, border: `1px solid ${theme.palette.divider}`, borderRadius: 2 }}>
                                ...
                                <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '2fr 1fr 1fr' }, gap: 2 }}>
                                    <TextField fullWidth label="Full Name" value={formData.name} onChange={e => handleFormChange('name', e.target.value)} error={!!formErrors.name} helperText={formErrors.name} required InputProps={{ startAdornment: <InputAdornment position="start"><Badge color="action" /></InputAdornment> }} />
                                    <TextField fullWidth label="First Name" value={formData.first_name} onChange={e => handleFormChange('first_name', e.target.value)} />
                                    <TextField fullWidth label="Last Name" value={formData.last_name} onChange={e => handleFormChange('last_name', e.target.value)} />
                                </Box>
                            </Paper>
                            ...
                            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 3 }}>
                                <Paper elevation={0} sx={{ p: 3, border: `1px solid ${theme.palette.divider}`, borderRadius: 2 }}>
                                    ...
                                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                                        <TextField fullWidth label="Email Address" type="email" value={formData.email} onChange={e => handleFormChange('email', e.target.value)} error={!!formErrors.email} helperText={formErrors.email} required InputProps={{ startAdornment: <InputAdornment position="start"><Email color="action" /></InputAdornment> }} />
                                        <TextField fullWidth label="Phone Number" type="number" value={formData.phone} onChange={e => handleFormChange('phone', e.target.value === '' ? '' : Number(e.target.value))} error={!!formErrors.phone} helperText={formErrors.phone} required InputProps={{ startAdornment: <InputAdornment position="start"><Phone color="action" /></InputAdornment> }} />
                                    </Box>
                                </Paper>
                                                ...
                                                endAdornment: (
                                                    <InputAdornment position="end">
                                                        <Tooltip title="Generate Password">
                                                            <IconButton onClick={handleGenerateFormPassword} color="primary"><Refresh /></IconButton>
                                                        </Tooltip>
                                                        {formData.password && (
                                                            <Tooltip title="Copy">
                                                                <IconButton onClick={() => copyToClipboard(formData.password)} color="success"><ContentCopy /></IconButton>
                                                            </Tooltip>
                                                        )}
                                                    </InputAdornment>
                                                ),
                            ...
                            {/* Customer link */}
                            <Paper elevation={0} sx={{ p: 3, border: `2px solid ${formErrors.customer_id ? theme.palette.error.main : theme.palette.primary.main}`, borderRadius: 2, bgcolor: alpha(theme.palette.primary.main, 0.02) }}>
                                    ...
                                    renderOption={(props, option) => (
                                        <li {...props} key={option._id || option.contact_id}>
                                            <Box display="flex" alignItems="center" gap={2} py={0.5}>
                                                <Avatar sx={{ bgcolor: 'primary.light', width: 32, height: 32 }}><Business fontSize="small" /></Avatar>
                                                <Box>
                ...
                <DialogActions sx={{ px: 3, py: 2, borderTop: `1px solid ${theme.palette.divider}` }}>
                    <Button onClick={() => setDialogOpen(false)} startIcon={<Cancel />} size="large">
                        {dialogMode === 'view' ? 'Close' : 'Cancel'}
                    </Button>
                    {dialogMode !== 'view' && (
                        <Button variant="contained" onClick={handleSubmit} disabled={submitting} startIcon={submitting ? <CircularProgress size={18} color="inherit" /> : <Save />} size="large" sx={{ px: 4 }}>
            ...
            {/* Reset password */}
            <Dialog open={resetDialogOpen} onClose={() => setResetDialogOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: 3 } }}>
                <DialogTitle>
                    <Box display="flex" alignItems="center" gap={2}>
                ...
                <DialogContent>
                    <TextField
                        ...
                        InputProps={{
                            ...
                            endAdornment: (
                                <InputAdornment position="end">
                                    <Tooltip title="Generate"><IconButton onClick={handleGenerateResetPassword} color="primary"><Refresh /></IconButton></Tooltip>
                                    {newPassword && <Tooltip title="Copy"><IconButton onClick={() => copyToClipboard(newPassword)} color="success"><ContentCopy /></IconButton></Tooltip>}
                                </InputAdornment>
                            ),
                        }}
                    />
                ...
                <DialogActions sx={{ p: 2 }}>
                    ...
                    <Button variant="contained" color="warning" onClick={handleResetPasswordConfirm} disabled={!newPassword} startIcon={<LockReset />}>Reset Password</Button>
                </DialogActions>
            </Dialog>

            {/* Share credentials */}
            <Dialog open={shareOpen} onClose={() => setShareOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: 3 } }}>
                <DialogTitle>
                    <Box display="flex" alignItems="center" gap={2}>
                ...
                <DialogContent>
                    <TextField fullWidth label="Email" value={userToShare?.email ?? ''} slotProps={{ input: { readOnly: true } }} sx={{ mt: 2, mb: 2 }} InputProps={{ startAdornment: <InputAdornment position="start"><Email color="action" /></InputAdornment> }} />
                    <TextField
                        fullWidth
                        label="Password"
                        type="text"
                ...
                <DialogActions sx={{ p: 2 }}>
                    ...
                    <Button variant="contained" color="success" startIcon={<ContentCopy />} onClick={handleCopyShareCred} disabled={!sharePwd}>Copy Credentials</Button>
                </DialogActions>
            </Dialog>
        </Box>
    );
